...
namespace RunnerMovementSystem
{
    public class MovementSystem : MonoBehaviour
    {
        ...
        [SerializeField] private float _maximumAccelerationTime = 1f;
        ...
        public void MoveForward(float runTime)
        {
            if (enabled)
            {
                float accelerate =  runTime/_maximumAccelerationTime;
                if (accelerate >= 1f)
                    accelerate = 1f;
                _movementBehaviour.SetAccelerate(accelerate);
